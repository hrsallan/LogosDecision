        ...
        try:
            ...
            try:
                ...
                notify_scraper_error(
                    where="Scheduler/Releitura",
                    err=e,
                    extra={
                        "auto_releitura": self.auto_releitura,
                        "janela": self._schedule_display(),
                    },
                )
            except Exception:
                pass
        ...
        try:
            ...
            if not downloaded_path or not os.path.exists(downloaded_path):
                ...
                try:
                    from core.email_alerts import notify_scraper_error
                    notify_scraper_error(
                        where="Scheduler/Porteira",
                        err=RuntimeError("Falha no download do relatório de porteira"),
                        extra={
    ...
    try:
        ...
        if not downloaded_path or not os.path.exists(downloaded_path):
            ...
            try:
                ...
                notify_scraper_error(
                    where="Scheduler/Releitura",
                    err=RuntimeError("Download falhou (releitura)"),
                    extra={"downloaded_path": downloaded_path, "manager": manager_username},
                    traceback_text="(sem traceback: download não gerou arquivo)",
                )
            except Exception:
                pass
