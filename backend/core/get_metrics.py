...
def get_porteira_stats(user_id):
    """
    ...
    cursor.execute("""
        ...
        FROM resultados_leitura
        WHERE user_id = ? AND Leituras_Nao_Executadas > 0
        GROUP BY Razao
    ...
    cursor.execute("""
        ...
        FROM resultados_leitura
        WHERE user_id = ? AND Porcentagem_Nao_Executada > 0
...
def get_porteira_efficiency_score(user_id):
    """
    ...
    """
    stats = get_porteira_stats(user_id)
...
def get_dashboard_metrics(user_id):
    ...
    return {
        ...
        'porteira': get_porteira_stats(user_id),
        'porteira_efficiency': get_porteira_efficiency_score(user_id),
