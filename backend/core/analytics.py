    ...
    try:
        ...
        while i < len(data_matrix):
            ...
            if re_ul.match(ul_val or '') and re_inst.match(inst_val or '') and re_data.match(data_val or ''):
                # Ignorar linhas com "Reg." como valor (é header)
        ...
        # NÃO descartar duplicatas - manter todos os registros (03 e Z3)
        # Apenas remover se instalação for "---"
        unique_details = []
        for d in details:
            if d['inst'] != "---":
...
def deep_scan_porteira_excel(file_path):
    ...
    Regras:
    - Não usa informações predefinidas (tudo vem do arquivo enviado/baixado)
    - Razão = 2 primeiros dígitos da UL (01..18)
    - Tudo que NÃO for razão 01 a 18 é descartado.
    """
    import tempfile
    import subprocess
    ...
    def _to_xlsx_if_needed(path_str: str) -> str:
        ...
        # Tentar ler via xlrd primeiro (se instalado no ambiente do usuário)
        try:
        ...
        # Fallback: converter via soffice (precisa do LibreOffice instalado no servidor)
        out_dir = Path(tempfile.mkdtemp(prefix="porteira_xls2xlsx_"))
        try:
            subprocess.run(
            ...
            # Se falhar, mantém original (vai estourar no read_excel com uma msg clara abaixo)
            return path_str

        return path_str

    try:
        # Ler o arquivo (suporta .xlsx; .xls precisa de xlrd ou conversão via soffice)
        normalized_path = _to_xlsx_if_needed(file_path)

        # Se for .xls e não converteu, pandas vai orientar instalar xlrd
        ...
        # Índices de colunas (confirmados no layout do relatório):
        # 0 = UL
        # 1 = Tipo UL (CNV/OSB)
        # 13 = Total (Leituras)
        # 16 = Leitura não executada
        # 50 = Releitura não executada
        # 52 = Releitura total
        COL_UL = 0
        COL_TIPO_UL = 1
        COL_TOTAL_LEIT = 13
        ...
        for i in range(len(df_raw)):
            ...
            # Detectar cabeçalho "Conjunto de Contrato"
            ...
            # Validar UL (8 dígitos)
            ul_clean = ul_val.replace(".0", "")
            if not ul_clean.isdigit() or len(ul_clean) != 8:
                continue

            # Tipo UL (CNV/OSB) - pode variar de coluna conforme export do portal
            tipo_ul_val = ""
            try:
                # Primeiro: coluna padrão ao lado da UL
                if len(row) > COL_TIPO_UL and pd.notna(row.iloc[COL_TIPO_UL]):
                    tipo_ul_val = str(row.iloc[COL_TIPO_UL]).strip()
                # Fallback: procurar CNV/OSB nas primeiras colunas da linha (relatórios às vezes deslocam)
                if not tipo_ul_val:
            ...
            if int(razao) < 1 or int(razao) > 18:
                # regra do usuário: descarte fora da razão 1 a 18
                continue
        ...
        # Consolidar (pode haver UL repetida por tipo OSB/CNV)
        ...
        # Retornar no formato esperado pela UI / DB
        details = []
        for _, r in df_grouped.iterrows():
            details.append({
                "Conjunto_Contrato": str(r["Conjunto_Contrato"]),
                "UL": str(r["UL"]),
                "Tipo_UL": str(r.get("Tipo_UL", "") if hasattr(r, 'get') else r["Tipo_UL"]),
